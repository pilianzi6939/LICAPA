...
Define_Module(ZigbeeProtocol);

void Protocolb::initialize()
{
    if (getIndex() == 0) {
    ...
    lastCommunicationTime = simTime();
}

void Protocolb::handleMessage(cMessage *msg)
{
...
void Protocolb::processInitialSignal(cMessage *msg)
{
...
void Protocolb::processAggregatedMessage(cMessage *msg)
{
...
void Protocolb::sendInitialSignal()
{
...
void Protocolb::aggregateAndSend()
{
...
void Protocolb::recordCommunicationTime()
{
    // Check if the specified time period has elapsed since the last communication
    simtime_t currentTime = simTime();
    simtime_t communicationInterval = getParentModule()->par("communicationInterval");
    if (currentTime - lastCommunicationTime >= communicationInterval) {
        // Record the communication time and nodeId
        EV_INFO << "Node " << getIndex() << " communicated at time: " << currentTime << endl;
